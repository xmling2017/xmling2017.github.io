...
Kubuntu的分区方案一般是，500MB的`EFI系统分区`，1GB的`\boot`，剩下的都放在根目录下（主文件与系统文件合并，**尽可能的放在固态硬盘**）。

### 更换软件源

安装后默认的软件源是国外的，如果不更换的话，下载软件包会很慢，有些驱动的下载也会报错。

这里统一换成中科大的镜像软件源。打开`/etc/apt/sources.list`，将文件内所有的`cn.archive.ubuntu.com`换成`mirrors.ustc.edu.cn`。保存后连接网络刷新一下软件源，命令`sudo apt update`（这是24版本之前的操作方式，之后的版本，有两个方法，一个是到`/usr/share/applications/desktop开头`，双击该文件，直接可视化操作；另一个方法参考[该网站](https://mirrors.tuna.tsinghua.edu.cn/help/ubuntu/)。LinuxMint系统更换软件源的方式可以参考[Linuxmint21.3使用体验分享](https://www.bilibili.com/video/BV1AC411L7ey/)）
...
{{% admonition info "提示" %}}
由于我们电脑开机的默认启动项都是Windows，这里只需要`sudo kate /etc/default/grub`，将文件中的启动数字改成对应的Windows的启动数字即可，然后更新文件`sudo update-grub`
{{% /admonition %}}
...
卸载 KUbuntu 自带的 Firefox `sudo snap remove --purge firefox`
...
从[官网](https://apps.ankiweb.net)下载Anki（一般选择「-qt6」），如果`zstd`还没有安装，则`sudo apt install zstd`
...
echo "export export PATH=/usr/local/lib/nodejs/bin:$PATH" >> ~/.profile
